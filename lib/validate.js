...
function validateContentShape(content) {
  ...
  if (!Array.isArray(content.hero.images) || content.hero.images.length === 0) return 'content.hero.images musí být neprázdné pole.';
  ...
  if (!content.legal.termsPage || !Array.isArray(content.legal.termsPage.sections)) return 'content.legal.termsPage musí obsahovat pole sections.';
  if (!content.legal.complaintsPage || !Array.isArray(content.legal.complaintsPage.sections)) return 'content.legal.complaintsPage musí obsahovat pole sections.';
  return null;
}
