...
function renderHeader(c) {
  ...
  return `<header class="site-header" id="siteHeader">
      <div class="container">
        <a href="#" class="logo">
          <img src="/assets/img/brand/logo-pz-mark.png" alt="" class="logo-mark" width="36" height="36">
...
function renderHero(h) {
  ...
  const slides = images.map((img, idx) =>
    `<div class="hero-slide"><img src="${esc(img.image)}" alt="${esc(img.alt)}" loading="${idx === 0 ? 'eager' : 'lazy'}" draggable="false"></div>`
  ).join('');
  const dots = multi
    ? `<div class="hero-slide-dots">${images.map((_, idx) => `<button type="button" class="hero-slide-dot ${idx === 0 ? 'is-active' : ''}" data-slide="${idx}" aria-label="Fotografie ${idx + 1}"></button>`).join('')}</div>`
    : '';
  const badge = h.badgeVisible !== false
  ...
  return `<section class="hero" id="hero">
      <div class="container hero-grid">
        ...
        <div class="hero-visual">
          <div class="hero-image-frame ${multi ? 'is-draggable' : ''}" id="heroSlider">
            <div class="hero-slider-track" id="heroSliderTrack">${slides}</div>
          </div>
          ${dots}
          ${badge}
        </div>
...
function renderAbout(a) {
  ...
  const statsItems = (a.stats || []).map(s => `
      <div class="stat-item reveal">
        <div class="stat-value">${esc(s.value)}</div>
        <div class="stat-label">${esc(s.label)}</div>
      </div>`).join('');
  const stats = statsItems ? `<div class="about-stats">${statsItems}</div>` : '';
...
function renderGallery(g) {
  ...
  const items = g.items.map((it, idx) => {
    const images = it.images && it.images.length ? it.images : [];
    const cover = images[0] || { image: '', alt: it.title };
    ...
    return `
      <figure class="masonry-item reveal" data-category="${esc(it.category)}" data-index="${idx}"
        ...
        data-description="${esc(it.description || '')}" data-images="${imagesJson}">
        <img src="${esc(cover.image)}" alt="${esc(cover.alt)}" loading="lazy">
        ${images.length > 1 ? `<span class="masonry-count">${images.length} fotky</span>` : ''}
  ...
  return `<section class="gallery section-pad" id="galerie">
    ...
    <div class="lightbox" id="lightbox" role="dialog" aria-modal="true" aria-label="Detail exponátu">
      ...
      <button class="lightbox-nav lightbox-prev" id="lightboxPrev" aria-label="Předchozí exponát">‹</button>
      <button class="lightbox-nav lightbox-next" id="lightboxNext" aria-label="Další exponát">›</button>
      <div class="lightbox-inner">
        <div class="lightbox-image">
          <div class="hero-image-frame" id="lightboxSlider">
            <div class="hero-slider-track" id="lightboxSliderTrack"></div>
          </div>
          <div class="hero-slide-dots" id="lightboxSlideDots"></div>
        </div>
        <div class="lightbox-plate">
          <div class="plate-tag" id="lightboxTag">Exponát</div>
          <h3 id="lightboxTitle"></h3>
          <p class="plate-description" id="lightboxDescription"></p>
...
function renderFooter(content) {
  ...
  const hasSocial = !!(f.facebookUrl || f.instagramUrl);
  const socialCol = hasSocial ? `
        <div class="footer-col">
          <h4 class="footer-col-title">${esc(f.socialLabel)}</h4>
          <div class="footer-social">
            ${f.facebookUrl ? `<a href="${esc(f.facebookUrl)}" target="_blank" rel="noopener" aria-label="Facebook" class="footer-social-icon">
              <svg viewBox="0 0 24 24" fill="currentColor"><path d="M13.5 21v-8.1h2.72l.4-3.16H13.5V7.7c0-.92.25-1.54 1.57-1.54H16.7V3.34c-.29-.04-1.27-.12-2.42-.12-2.4 0-4.04 1.46-4.04 4.15v2.31H7.5v3.16h2.74V21h3.26z"/></svg>
            </a>` : ''}
            ${f.instagramUrl ? `<a href="${esc(f.instagramUrl)}" target="_blank" rel="noopener" aria-label="Instagram" class="footer-social-icon">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><rect x="3" y="3" width="18" height="18" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.3" cy="6.7" r="0.9" fill="currentColor" stroke="none"/></svg>
            </a>` : ''}
          </div>
        </div>` : '';
  return `<footer class="site-footer">
      <div class="container footer-grid-top ${hasSocial ? 'has-social' : ''}">
      ...
      <div class="container footer-bottom">
        ...
        <div class="footer-bottom-right">
          <a href="/obchodni-podminky.html">${esc(content.legal.footerTermsLabel || 'Obchodní podmínky')}</a>
          <a href="/reklamacni-rad.html">${esc(content.legal.footerComplaintsLabel || 'Reklamační řád')}</a>
...
function renderLegalArticle(page) {
  ...
  return `<article class="legal-page section-pad">
      <div class="container legal-container">
        ...
        <p class="legal-updated">Poslední aktualizace: ${esc(page.updated)}</p>
        <p class="legal-intro">${esc(page.intro)}</p>
        ${sections}
      </div>
    </article>`;
}
function renderPrivacyArticle(legal) { return renderLegalArticle(legal.privacyPage); }
...
function renderNotFoundArticle(content) {
  const n = content.notFound;
  const links = content.nav.links.map(l => `<a class="notfound-link" href="/#${esc(l.target)}">${esc(l.label)}</a>`).join('');
  return `<article class="notfound-page section-pad">
      <div class="container notfound-container">
        <p class="eyebrow">${esc(n.eyebrow)}</p>
        <div class="notfound-code">${esc(n.code)}</div>
        <h1 class="section-title">${esc(n.title)}</h1>
        <p class="notfound-message">${esc(n.message)}</p>
        <div class="notfound-cta">
          <a class="btn btn-primary" href="/">${esc(n.homeLabel)}</a>
        </div>
        <div class="notfound-links">
          <span class="notfound-links-label">${esc(n.linksLabel)}</span>
          <div class="notfound-links-list">${links}</div>
        </div>
      </div>
    </article>`;
}

function buildLegalHtml(content, theme, page, urlPath) {
  const base = content.seo.canonicalUrl || 'https://platnerstvi-zatrapa.cz/';
  const pageUrl = base.replace(/\/$/, '') + '/' + urlPath;
  const title = `${page.title} — ${content.site.name}`;

  const body = [
    renderHeader(content),
    '<main id="main">',
    renderLegalArticle(page),
    '</main>',
    renderFooter(content),
    renderCookieBanner(content.legal)
  ].join('\n');
...
function buildPrivacyHtml(content, theme) { return buildLegalHtml(content, theme, content.legal.privacyPage, 'ochrana-osobnich-udaju.html'); }
function buildTermsHtml(content, theme) { return buildLegalHtml(content, theme, content.legal.termsPage, 'obchodni-podminky.html'); }
function buildComplaintsHtml(content, theme) { return buildLegalHtml(content, theme, content.legal.complaintsPage, 'reklamacni-rad.html'); }

/* ---------------- Stránka 404 ---------------- */

function buildNotFoundHtml(content, theme) {
  const n = content.notFound;
  const title = `${n.title} — ${content.site.name}`;

  const body = [
    renderHeader(content),
    '<main id="main">',
    renderNotFoundArticle(content),
    '</main>',
    renderFooter(content),
    renderCookieBanner(content.legal)
  ].join('\n');
...
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>${esc(title)}</title>
<meta name="description" content="${esc(n.message)}">
<meta name="robots" content="noindex, follow">
...
module.exports = {
  ...
  renderCookieBanner, renderPrivacyArticle, renderLegalArticle, renderNotFoundArticle,
  renderThemeStyle, absUrl, buildIndexHtml, buildPrivacyHtml, buildTermsHtml, buildComplaintsHtml, buildNotFoundHtml
};
