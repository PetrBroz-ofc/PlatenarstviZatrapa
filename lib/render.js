...
function renderHero(h) {
  const buttons = h.buttons.map(b =>
    `<a class="btn btn-${b.style === 'primary' ? 'primary' : 'ghost'}" href="#${esc(b.target)}">${esc(b.label)}</a>`
  ).join('');
  return `<section class="hero section-pad" id="hero">
  ...
  return `<footer class="site-footer">
      <div class="container footer-grid-top">
        ...
        <div class="footer-col">
          ...
          <div class="footer-contact-name">${esc(content.contact.name)}</div>
          ...
          <div class="footer-contact-line">
            ...
            <span>${esc(content.contact.city)}</span>
          </div>
        </div>
      </div>
...
module.exports = {
  ...
  renderThemeStyle, absUrl, buildIndexHtml
};
